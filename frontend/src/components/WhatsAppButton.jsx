...
import { useContextualWhatsAppLink } from "../lib/whatsapp.js";

// The href is now built per-page (see lib/whatsapp.js) instead of a single
// fixed WHATSAPP_LINK, so tapping this button always opens WhatsApp with a
// message that already states what the visitor was looking at - a project,
// amenities, the calculator, etc. - instead of the same blank greeting
// from every page.
export default function WhatsAppButton({ hidden = false }) {
  const { t } = useTranslation();
  const href = useContextualWhatsAppLink();
  return (
    <a
      href={href}
      target="_blank"
